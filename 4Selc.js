...
async function BookOrdsystem() {
    let mStatus = await inquirer.prompt([
        {
            name: "selectedOption",
            type: "list",
            message: "For Membership status Please select an type",
            choices: ["Regular customer", "Silver memeber", "Gold member", "Platinum member"]
        },
        {
            name: "bookNum",
            type: "number",
            messsage: " Please enter Number of books purchased"
        }
    ]);
    let unitPrice = 100;
    let status = mStatus.selectedOption;
    let bookPurchased = mStatus.bookNum;
    let regCustomer = true;
    if (status == regCustomer) {
        let totalBill = bookPurchased * unitPrice;
        console.log(`Your total bill is : ${totalBill} `);
        console.log("Regular Customer get no discount");
    }
}
;
BookOrdsystem();
